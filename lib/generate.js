 ...
 */
function generate(detailsfile /* file to details.xml*/, modulename /*module object called*/, config, cb) {
    getDetails(detailsfile, modulename, config.version, function (pageObject) {
        process.nextTick(function () {
            ...
            if (pageObject.methods && pageObject.methods.length > 0) {
                ...
                retObjectItem.methods = getMethodOrEventObjects(pageObject.methods); // allow null
            }
            if (pageObject.events && pageObject.events.length) {
                ...
                retObjectItem.events = getMethodOrEventObjects(pageObject.events); // allow null
            }
...
function getMethodOrEventObjects(methods) {
    var obj, lclMethods = [], lclParms = [], usage = null;
    methods.forEach(function (item) {
        obj = {name: item.name, scope: item.scope, type: item.type, types: item.types, from: item.from, summary: item.summary};
        var shortSummary = "";
        if (item.summary) {
            shortSummary = trimSummary(item.summary, true);
        }

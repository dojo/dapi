            ...
            process.nextTick(function(){
                ...
                if (pageObject.properties && pageObject.properties[0].property){
                    ...
                    pageObject.properties[0].property.forEach(function(item){
                        lclProps.push({name:item.$.name, scope:item.$.scope, type:item.$.type, from:item.$.from, summary:item.summary});
                    });
                    retObjectItem.properties = lclProps; // allow empty
                }
                // end generate properties
                ...
                if (methods.length > 0){ //this is long, make reusable for events?
                    retObjectItem.methods = getMethodOrEventObjects(methods); // will be null??
                }
                if (events.length > 0){
                    retObjectItem.events = getMethodOrEventObjects(events); // will be null??
                }

...
/**
 * Get the 'pageObject' object, this is the module object from details.json located by the version and modulename
 * @private 
...
function hyperlink(text, label, config, moduleversion) {
    ...
    if (url) {
        return '<a class="jsdoc-link" href="' + config.contextPath + config.apiDataPath + '/' + moduleversion + '/' + url + '">'
            + (label || text)
            + '</a>';
    } else {
        // Word like "Boolean"
...
/**
 * Parse any text to convert any module type paths to links which the api parser can link to. This is intended to be used by JADE templates.   
...
function autoHyperlink(text, config, moduleversion) {
    ...
    splitTextArr.map(function (part) {
        if (part === "<pre><code>") {
            inExample = true;
            returnText += "<pre><code>";
...
/**
 * Get an icon name to be used for a property type. This is intended to be used by templates
 * @public 
